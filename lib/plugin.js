      ...
      taskMap[heading].forEach(task => {
        ...
        taskContent.addEventListener('blur', () => {
          ...
          if (newContent !== task.content) {
            ...
            // Optionally, you can update the backend
...
`;
  ...
  generateMarkdown(taskMap) {
    ...
    Object.keys(taskMap).forEach(heading => {
      // Add the heading with Markdown formatting
      markdown += `# ${heading}\n\n`;

      // Loop through tasks in the current heading and add each task in Markdown format
      taskMap[heading].forEach(task => {
    ...
    return markdown.trim(); // Trim any extra spaces or line breaks at the end
  },
